...
function attachGlobalListeners() {
  ...
  document.getElementById("printWorksheetBtn").addEventListener("click", printCurrentWorksheet);
...
async function generateReteachWorksheet(kid, sourceWorksheet, grading) {
  // If the marked sheet was a real template, rebuild the SAME kind of sheet (same
  // skill, same setup, optionally eased) so the re-teach is unmistakably tied to
  // what was just marked — not a generic, unrelated AI question list.
  if (sourceWorksheet && sourceWorksheet.templateId && window.TEMPLATES[sourceWorksheet.templateId]) {
    return buildTemplateReteach(kid, sourceWorksheet, grading);
  }

  // Otherwise (an AI/question-list sheet) generate a targeted AI re-teach.
  ...
  return {
    ...
    reteachOfCode: sourceWorksheet ? wsCode(sourceWorksheet) : null,
    notes: sourceWorksheet ? `Re-teach of ${wsCode(sourceWorksheet)} — same skill, fresh practice.` : "Re-teach worksheet.",
    generatedAt: Date.now()
  };
}

// Re-teach for a template-based sheet: re-run the SAME template with the same setup
// (fresh problems), easing a few common difficulty knobs when the mark says "easier".
function buildTemplateReteach(kid, sourceWorksheet, grading) {
  const t = window.TEMPLATES[sourceWorksheet.templateId];
  const mods = Object.assign({}, sourceWorksheet.modifiers || {});
  if (grading && grading.recommendation === "easier") easeTemplateModifiers(mods);
  const content = t.generate(mods);
  return {
    id: newWorksheetId(),
    kidId: kid.id,
    subject: sourceWorksheet.subject,
    templateId: sourceWorksheet.templateId,   // renders as the REAL worksheet, same as the original
    modifiers: mods,
    content,
    title: sourceWorksheet.title,             // same kind of sheet
    difficulty: sourceWorksheet.difficulty,
    standards: sourceWorksheet.standards || [],
    reteachOf: sourceWorksheet.id,
    reteachOfCode: wsCode(sourceWorksheet),
    notes: `Re-teach of ${wsCode(sourceWorksheet)} — same template, fresh practice${grading && grading.recommendation === "easier" ? " (eased)" : ""}.`,
    generatedAt: Date.now()
  };
}

// Best-effort easing: only nudges modifier keys that actually exist on this template.
function easeTemplateModifiers(m) {
  const num = (k, factor, floor) => {
    if (m[k] == null) return;
    const n = parseInt(m[k], 10);
    if (!isNaN(n)) m[k] = Math.max(floor, Math.round(n * factor));
  };
  num("count", 0.7, 4);
  num("maxWords", 0.7, 3);
  num("maxValue", 0.5, 5);
  num("max", 0.5, 5);
  const stepDown = { "4": "3", "3": "2", "2": "1" };
  if (m.digits != null && stepDown[String(m.digits)]) m.digits = stepDown[String(m.digits)];
  if (m.stackHeight != null && stepDown[String(m.stackHeight)]) m.stackHeight = stepDown[String(m.stackHeight)];
  return m;
}
...
function printCurrentWorksheet() {
  if (!currentWorksheetForModal) return;
  commitWorksheetIfNew(currentWorksheetForModal);
  printWorksheet(currentWorksheetForModal, { includeAnswers: false });
}
...
// Date-stamped, per-kid filename so downloads never overwrite each other.
function pdfFilename(ws, includeAnswers) {
  const kid = state.kids[ws.kidId];
  const now = new Date();
  const pad = n => String(n).padStart(2, "0");
  const stamp = `${now.getFullYear()}-${pad(now.getMonth() + 1)}-${pad(now.getDate())}_${pad(now.getHours())}${pad(now.getMinutes())}`;
  return [includeAnswers ? "answer_key" : "worksheet", slugify(kid ? kid.name : ""), slugify(ws.title), stamp]
    .filter(Boolean).join("_") + ".pdf";
}

// Download: build the doc, then save with a date-stamped name.
function buildPDF(ws, { includeAnswers }) {
  const doc = buildPDFDoc(ws, { includeAnswers });
  if (doc) doc.save(pdfFilename(ws, includeAnswers));
}

// Print: build the doc and open the Mac print dialog via a hidden iframe.
...
// Builds and returns the jsPDF doc (no save) — shared by download + print.
function buildPDFDoc(ws, { includeAnswers }) {
  ...
  // Provenance code stamped into the footer so any printed/scanned sheet is traceable.
  window.__wsCode = wsCode(ws);
  ...
  const codeSuffix = window.__wsCode ? "  •  " + window.__wsCode : "";
  doc.text("Generated by SOVRN Homeschool HQ • BC Curriculum aligned" + codeSuffix, margin, doc.internal.pageSize.getHeight() - 24);
...
// Worksheet ids currently being marked in the background (so the Grading list can
// show a "marking…" badge and prevent double-submits while the user moves on).
const _gradingInProgress = new Set();
...
// Marking runs in the BACKGROUND: we snapshot the file + target sheet, close the
// modal right away, and let the user move on to the next upload. The Grading row
// shows a "marking…" badge and flips to the score when Claude returns. Multiple
// sheets can be marked at once.
function runGrading() {
  if (!currentGradeFile) return;

  // Snapshot inputs NOW — module-level currentGrade* get overwritten the moment the
  // user opens the next sheet, so the background task must not read them later.
  const file = currentGradeFile;
  const worksheetId = currentGradeWorksheetId;
  const worksheet = worksheetId ? findWorksheet(worksheetId) : null;
  const label = worksheet ? (worksheet.title || "worksheet") : "worksheet";
  const progressKey = worksheetId || ("adhoc_" + Date.now());

  // Close the modal and show the row as "marking…" immediately.
  closeAllModals();
  _gradingInProgress.add(progressKey);
  renderContent();
  toast(`Marking “${label}” in the background…`, "success");
  ...
  (async () => {
    try {
      ...
      console.error(e);
      toast(`Marking “${label}” failed: ${e.message}`, "error");
    } finally {
      _gradingInProgress.delete(progressKey);
      renderContent(); // flip the row to its score (or back to "to mark" on failure)
    }
  })();
}
...
function buildGradingPrompt(worksheet) {
  ...
  // Provenance: the exact source sheet, so the mark is unambiguously tied to it.
  const ident = worksheet
    ? `This is "${worksheet.title || "a worksheet"}" (reference code ${wsCode(worksheet)}, printed in the page footer). Mark ONLY this sheet; if the photo's footer code clearly differs, say so in your notes.`
    : "";
...
function mockGradingResponse(worksheet) {
  ...
  return {
    score,
    perQuestion: (worksheet && Array.isArray(worksheet.questions)) ? worksheet.questions.map((q, i) => ({
...
function gradingRowHTML(w) {
  ...
  const marking = _gradingInProgress.has(w.id);
  let badge;
  if (marking) {
    badge = `<span class="tag" style="background:#e7eef6; color:#2c5b8a;"><span class="spinner spinner-sm"></span> marking…</span>`;
  } else if (st.state === "graded") {
  ...
  const markBtn = `<button class="btn btn-secondary" data-action="grade" data-worksheet-id="${w.id}" ${marking ? "disabled" : ""} title="Upload the completed sheet & mark it">${st.state === "graded" ? "View / re-mark" : "Mark"}</button>`;
  ...
  return `
    <div class="history-item">
      ...
      <div class="meta" data-action="view-ws" data-worksheet-id="${w.id}" style="cursor:pointer;" title="Click to preview this sheet">
        ...
        <div class="meta-sub">${nameHTML} • ${capitalize(w.subject)}${w.difficulty ? " • Difficulty " + w.difficulty : ""} • <span class="muted" style="font-variant-numeric:tabular-nums;">${wsCode(w)}</span></div>
      </div>
      ${badge}
      ${markBtn}
      ${reprintBtn}
...
function exportPortfolioPDF(kid) {
  ...
  doc.save(`portfolio_${slugify(kid.name)}_${new Date().toISOString().slice(0, 10)}.pdf`);
}
...
function openSettings() {
  ...
  if (vLine) {
    ...
    vLine.textContent = v ? `SOVRN Homeschool HQ v${v}` : "SOVRN Homeschool HQ • browser preview";
  }
...
// Fresh unique worksheet id.
function newWorksheetId() {
  return "ws_" + Date.now() + "_" + Math.random().toString(36).slice(2, 7);
}

// Stable, human-readable provenance code for a worksheet, derived from its id.
// Printed in the PDF footer + shown in the Grading list so the exact source sheet
// is always identifiable (and so a re-teach is unmistakably tied to what was marked).
function wsCode(ws) {
  if (!ws) return "";
  if (ws.code) return ws.code;                 // honor an explicit code if ever set
  const base = String(ws.id || "");
  let h = 0;
  for (let i = 0; i < base.length; i++) h = (h * 31 + base.charCodeAt(i)) >>> 0;
  const s = h.toString(36).toUpperCase().replace(/[^A-Z0-9]/g, "").padStart(5, "0").slice(-5);
  return "HQ-" + s;
}
